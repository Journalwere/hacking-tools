...
function cleanup() {
    ...
    rm -f ffuf_results.txt &> /dev/null
...
for tool in "${selected_tools[@]}"; do
    case $tool in
        "ffuf")
            gnome-terminal --title="FFUF" -- bash -c "ffuf -c -w \"$ffuf_wordlist_path\" -u \"$protocol$url\" -H \"Host: FUZZ.$url\" -fl 8 | tee ffuf_results.txt; read -p 'Press Enter to close this terminal'"
            ;;
        "gobuster")
            echo "Enter a filetype for gobuster:"
            read filetype

            gnome-terminal --title="Gobuster" -- bash -c "gobuster dir -w \"$gobuster_wordlist_path\" -u \"$protocol$url\" -x \"$filetype\" | tee gobuster_results.txt; read -p 'Press Enter to close this terminal'"
            ;;
        "nikto")
        ...
        "nmap")
            for ip in "${ip_addresses[@]}"; do
                gnome-terminal --title="Nmap $ip" -- bash -c "sudo nmap -p- -sV -O -T4 \"$ip\" -oX scan.xml | tee nmap_results.txt; read -p 'Press Enter to close this terminal'"
...
# Combine nmap, ffuf, gobuster, and searchsploit results
cat nmap_results.txt ffuf_results.txt gobuster_results.txt searchsploit_results.txt > comprehensive_report.txt
